  ...
  sdf::ElementPtr _sdf)
{
  ...
  if (this->sdf->HasElement("acs_imu"))
  {
    // Add names to map
    cameraImuSensorName = sdf->Get<std::string>("acs_imu");
  }
...
void GimbalControllerPlugin::OnUpdate()
{
  /// currentAngleYPRVariable is defined in roll-pitch-yaw-fixed-axis
    ...
    ignition::math::Vector3d currentAngleYPRVariable(
      ...
      //std::cout << "R: " << currentAngleYPRVariable.X() << "  ";
      //std::cout << "P: " << currentAngleYPRVariable.Y() << "  ";
      //std::cout << "Y: " << currentAngleYPRVariable.Z() << "\n";
  ...
  else if (time > this->lastUpdateTime)
  {
    ...
    */
    ...
    if(yawTarget < 0){
        ...
        acs_ccw[0].link->AddRelativeForce(acs_ccw[0].force);
        acs_ccw[1].link->AddRelativeForce(acs_ccw[1].force);
    }
    */
    /* force = {roll_calculated_x, roll_calculated_y, roll_calculated_z}
       link->AddRelativeForce(force);
       force = {pitch_calculated_x, pitch_calculated_y, pitch_calculated_z}
       link->AddRelativeForce(force);
    */
