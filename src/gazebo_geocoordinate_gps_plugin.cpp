...
#include <gazebo_geocoordinate_gps_plugin.h>
...
namespace gazebo {
GZ_REGISTER_SENSOR_PLUGIN(GeocoordinateGpsPlugin)

GeocoordinateGpsPlugin::GeocoordinateGpsPlugin() : SensorPlugin()
{ }

GeocoordinateGpsPlugin::~GeocoordinateGpsPlugin()
{
...
void GeocoordinateGpsPlugin::Load(sensors::SensorPtr _parent, sdf::ElementPtr _sdf)
{
  // Get then name of the parent sensor
  parentSensor_ = std::dynamic_pointer_cast<sensors::GpsSensor>(_parent);

  if (!parentSensor_)
    gzthrow("GeocoordinateGpsPlugin requires a GPS Sensor as its parent");
  ...
  updateSensorConnection_ = parentSensor_->ConnectUpdated(boost::bind(&GeocoordinateGpsPlugin::OnSensorUpdate, this));
  parentSensor_->SetActive(true);

  // Listen to the update event. This event is broadcast every simulation iteration.
  updateWorldConnection_ = event::Events::ConnectWorldUpdateBegin(
      boost::bind(&GeocoordinateGpsPlugin::OnWorldUpdate, this, _1));
...
void GeocoordinateGpsPlugin::OnWorldUpdate(const common::UpdateInfo& /*_info*/)
{
  ...
  ///TODO: Project LV03 to WGS84
  auto pos_with_noise = pos_W_I + noise_gps_pos_ + gps_bias_;
  auto latlon = reproject(pos_with_noise, lat_home_, lon_home_, alt_home_);

  Eigen::Vector3d lv03_origin;
  forward(lat_home_*180.0/M_PI, lon_home_*180.0/M_PI, alt_home_, lv03_origin.x(), lv03_origin.y(), lv03_origin.z());
  auto lv03_pos_with_noise = ignition::math::Vector3d(lv03_origin.x(), lv03_origin.y(), lv03_origin.z()) + pos_with_noise;
  Eigen::Vector3d lla_position;
  reverse(lv03_pos_with_noise.X(), lv03_pos_with_noise.Y(), lv03_pos_with_noise.Z(), lla_position.x(), lla_position.y(), lla_position.z());
  // fill SITLGps msg
  ...
  gps_msg.set_latitude_deg(lla_position.x());
  gps_msg.set_longitude_deg(lla_position.y());
  gps_msg.set_altitude(lla_position.z());
...
void GeocoordinateGpsPlugin::OnSensorUpdate()
{
  // protect shared variables
  std::lock_guard<std::mutex> lock(data_mutex_);
