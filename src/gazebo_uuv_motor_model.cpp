...
#include "gazebo_uuv_motor_model.h"
...
void GazeboUUVMotorModel::InitializeParams() {}

void GazeboUUVMotorModel::Publish() {}

void GazeboUUVMotorModel::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  model_ = _model;
  ...
  updateConnection_ = event::Events::ConnectWorldUpdateBegin(boost::bind(&GazeboUUVMotorModel::OnUpdate, this, _1));

  command_sub_ = node_handle_->Subscribe<mav_msgs::msgs::CommandMotorSpeed>("~/" + model_->GetName() + command_sub_topic_, &GazeboUUVMotorModel::VelocityCallback, this);
  }

// This gets called by the world update start event.
void GazeboUUVMotorModel::OnUpdate(const common::UpdateInfo& _info) {
...
void GazeboUUVMotorModel::VelocityCallback(CommandMotorSpeedPtr &rot_velocities) {
...
void GazeboUUVMotorModel::UpdateForcesAndMoments() {
    ...
    //Print forces
    //std::cout << std::setprecision(4) << "Forces: " << forces[0] << ", " << forces[1] << ", " << forces[2] << ", " << forces[3] << "\n ";
    //Print torques
    //std::cout << std::setprecision(4) << "Torques: " << torques[0] << ", " << torques[1] << ", " << torques[2] << ", " << torques[3] << "\n ";
    ...
    //Calculate and add hydrodynamic forces
     ...
     //Print velocities
     //std::cout << std::setprecision(4) << "Vel Linear:" << linear_velocity << "\n" << "Vel Angular:" << angular_velocity << "\n";
...
GZ_REGISTER_MODEL_PLUGIN(GazeboUUVMotorModel);
}
