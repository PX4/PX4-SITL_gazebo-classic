...
void GazeboMavlinkInterface::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  if (_sdf->HasElement("mavlink_udp_port")) {
    mavlink_udp_port = _sdf->GetElement("mavlink_udp_port")->Get<int>();
  ...
  if (_sdf->HasElement("mavlink_tcp_port")) {
    mavlink_tcp_port = _sdf->GetElement("mavlink_tcp_port")->Get<int>();
  }
  ...
  if (serial_enabled) {
    ...
    {
      std::string device_= _sdf->GetElement("serialDevice")->Get<std::string>();
      //TODO: Set device_
    }
