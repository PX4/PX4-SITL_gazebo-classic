...
void arucoMarkerPlugin::Load(sensors::SensorPtr _sensor, sdf::ElementPtr _sdf)
{
  ...
  int dict_id = 0; 
  this->marker_dict = makePtr<aruco::Dictionary>(cv::aruco::getPredefinedDictionary(cv::aruco::DICT_4X4_50)); //(cv::aruco::PREDEFINED_DICTIONARY_NAME(dict_id));  
  this->detectorParams = makePtr<aruco::DetectorParameters>(cv::aruco::DetectorParameters()); //::create();
}
