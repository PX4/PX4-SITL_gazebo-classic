...
#include "gazebo_motor_failure_model.h"
...
void GazeboMotorModel::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  if (_sdf->HasElement("robotNamespace"))
    ...
    gzerr << "[gazebo_motor_failure_model] Please specify a robotNamespace.\n";
  ...
  if (_sdf->HasElement("jointName"))
    ...
    gzerr << "[gazebo_motor_failure_model] Please specify a jointName, where the rotor is attached.\n";
  // Get the pointer to the joint.
  joint_ = model_->GetJoint(joint_name_);
  if (joint_ == NULL)
    gzthrow("[gazebo_motor_failure_model] Couldn't find specified joint \"" << joint_name_ << "\".");
  ...
  if (_sdf->HasElement("linkName"))
    ...
    gzerr << "[gazebo_motor_failure_model] Please specify a linkName of the rotor.\n";
  link_ = model_->GetLink(link_name_);
  if (link_ == NULL)
    gzthrow("[gazebo_motor_failure_model] Couldn't find specified link \"" << link_name_ << "\".");


  if (_sdf->HasElement("motorNumber"))
    motor_number_ = _sdf->GetElement("motorNumber")->Get<int>();
  else
    gzerr << "[gazebo_motor_failure_model] Please specify a motorNumber.\n";
  ...
  if (_sdf->HasElement("turningDirection")) {
    ...
    if (turning_direction == "cw")
      ...
      gzerr << "[gazebo_motor_failure_model] Please only use 'cw' or 'ccw' as turningDirection.\n";
  }
  else
    gzerr << "[gazebo_motor_failure_model] Please specify a turning direction ('cw' or 'ccw').\n";
  ...
  */
  ...
  if (!ros::isInitialized())  {
    ...
    gzwarn << "[gazebo_motor_failure_model]: Subscribe to /gazebo/motor_failure \n" ;
  }
