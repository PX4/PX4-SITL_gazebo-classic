                     ...
                     sdf::ElementPtr _sdf)
{
  ...
  if (_sdf->HasElement("propeller_wind_map")) {
    sdf::ElementPtr propMapElem = _sdf->GetElement("propeller_wind_map");

    if (propMapElem->HasElement("wash_only")) {
        this->wash_only_ = propMapElem->Get<bool>("wash_only");
    }

    sdf::ElementPtr propElem = propMapElem->GetElement("propeller");

    while (propElem) {
      std::string jointName = propElem->Get<std::string>("joint");
      double kv = propElem->Get<double>("k_v");
      double SlowdownSim = propElem->Get<double>("rotorVelocitySlowdownSim");

      physics::JointPtr joint = _model->GetJoint(jointName);

      if (joint) {
        this->propeller_joint_.push_back(joint);
        this->propeller_wind_constant_.push_back(kv);
        this->rotor_velocity_slowdown_sim_.push_back(SlowdownSim);
        this->num_of_propeller_++;

        this->HasPropellerWind_ = true;
      } else {
        gzerr << "[LiftDragPlugin] Propeller joint [" << jointName << "] not found!\n";
      }

      propElem = propElem->GetNextElement("propeller");
    }
  }
}
...
void LiftDragPlugin::OnUpdate()
{
...
#if GAZEBO_MAJOR_VERSION >= 9
  ignition::math::Vector3d link_vel_world = this->link->WorldLinearVel(this->cp);
  const common::Time current_time = this->world->SimTime();
#else
  ignition::math::Vector3d link_vel_world = ignitionFromGazeboMath(this->link->GetWorldLinearVel(this->cp));
  const common::Time current_time = this->world->GetSimTime();
#endif
  // Calculate the washing flow of propeller
  ignition::math::Vector3d wash_vel_world(0, 0, 0);
  if (this->HasPropellerWind_)
  {
    for (int i = 0; i < this->num_of_propeller_; ++i)
    {
      double omega = this->propeller_joint_[i]->GetVelocity(0) * this->rotor_velocity_slowdown_sim_[i];
      double speed_mag = this->propeller_wind_constant_[i] * fabs(omega);
      ignition::math::Vector3d axis = this->propeller_joint_[i]->GlobalAxis(0).Normalize();
  ...
  if (this->wash_only_) {
    // Consider only the wind speed generated by the propeller wash, ignoring the link's motion speed and environmental wind speed
    air_velocity = -wash_vel_world;
  } else {
  ...
  if (forwardI.Dot(air_velocity) <= 0.0){
  ...
  //
  // so,
  // removing spanwise velocity from air_velocity
  ignition::math::Vector3d velInLDPlane = air_velocity - air_velocity.Dot(spanwiseI)*spanwiseI;
  ...
  if (0)
  {
    ...
    gzdbg << "Link: [" << this->link->GetName()
          << "] pose: [" << pose
          << "] dynamic pressure: [" << q << "]\n";
    gzdbg << "spd: [" << air_velocity.Length()
          << "] air_velocity: [" << air_velocity << "]\n";
    gzdbg << "LD plane spd: [" << velInLDPlane.Length()
          << "] air_velocity : [" << velInLDPlane << "]\n";
