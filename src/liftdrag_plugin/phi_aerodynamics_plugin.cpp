...
#include "liftdrag_plugin/phi_aerodynamics_plugin.h"

#include "Force.pb.h"

using namespace gazebo;

GZ_REGISTER_MODEL_PLUGIN(PhiAerodynamics)

/////////////////////////////////////////////////
PhiAerodynamics::PhiAerodynamics() : cla(1.0), cda(0.01), cma(0.0), rho(1.2041)
{
...
/////////////////////////////////////////////////
PhiAerodynamics::~PhiAerodynamics()
{
}

/////////////////////////////////////////////////
void PhiAerodynamics::Load(physics::ModelPtr _model,
                     sdf::ElementPtr _sdf)
{
  GZ_ASSERT(_model, "PhiAerodynamics _model pointer is NULL");
  GZ_ASSERT(_sdf, "PhiAerodynamics _sdf pointer is NULL");
  this->model = _model;
  this->sdf = _sdf;

  this->world = this->model->GetWorld();
  GZ_ASSERT(this->world, "PhiAerodynamics world pointer is NULL");
...
#endif
  GZ_ASSERT(this->physics, "PhiAerodynamics physics pointer is NULL");
  ...
  if (_sdf->HasElement("link_name"))
  {
    ...
    if (!this->link)
    {
      gzerr << "Link with name[" << linkName << "] not found. "
        << "The PhiAerodynamics will not generate forces\n";
    }
    else
    {
      this->updateConnection = event::Events::ConnectWorldUpdateBegin(
          boost::bind(&PhiAerodynamics::OnUpdate, this));
    }
  }
  ...
  if (_sdf->HasElement("windSubTopic")){
    ...
    wind_sub_ = node_handle_->Subscribe("~/" + wind_sub_topic_, &PhiAerodynamics::WindVelocityCallback, this);
  }
...
void PhiAerodynamics::WindVelocityCallback(const boost::shared_ptr<const physics_msgs::msgs::Wind> &msg) {
